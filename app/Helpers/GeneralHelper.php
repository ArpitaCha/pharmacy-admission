...
if (!function_exists('sessionYear')) {
    ...
    {
        $a = $year;
        $b = substr($year + 1, -2); // Last two digits of next year
...
if (!function_exists('getOverallStatus')) {
    ...
    {
        $student =   DB::table('jexpo_register_student as rs')
            ->where('rs.s_id', $stud_id)->first();

        $allotment    =    DB::table('jexpo_choice_student')
        ...
        //echo sizeof($allotment);exit();

        $status = "PROFILE_NOT_UPDATED";

        //if ALLOTMENT EXIST

        if ($is_allot) {
            /*if(($student->is_lock_manual == 1) && ($student->is_payment == 1) 
                && ($student->is_alloted == 1) && ($student->is_allotment_accept == 0) 
                && ($student->is_upgrade == 0) && ($student->is_upgrade_payment == 1) 
				&& ($allotment->ch_auto_upgrade == 1)) {
				
				$status = "ALLOTED_SAME_CHOICE_NOT_UPGRADED";
			}
			else if(($student->is_lock_manual == 1) && ($student->is_payment == 1) 
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 0) 
					&& ($student->is_upgrade == 0) && ($student->is_upgrade_payment == 1) 
					&& ($allotment->ch_auto_upgrade == 0)) {
				
				$status = "ALLOTED_SAME_CHOICE_UPGRADED";
			}
			else if( ($student->is_lock_manual == 1) && ($student->is_payment == 1) 
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 1) 
					&& ($student->s_admited_status == 1)) {
				
				$status = "ADMISSION_CONFIRMED";
			}
			else if(($student->is_lock_manual == 1) && ($student->is_payment == 1) 
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 1) 
					&& ($student->s_admited_status == 2) && ($student->s_auto_reject == 0)) {
				$status = "ADMISSION_REJECTED";
			}
			else if(($student->is_lock_manual == 1) && ($student->is_payment == 1) 
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 1) 
					&& ($student->s_admited_status == 2) && ($student->s_auto_reject == 1)) {
					$status = "NO_ACTION_TAKEN_ON_ALLOTMENT";
			} 
			else if(($student->is_lock_manual == 1) && ($student->is_payment == 1) 
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 1) 
				...
				$status = "ALLOTMENT_ACCEPTED";
					...
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 0) 
					&& ($student->is_upgrade_payment == 1)) {
				$status = "ALLOTMENT_UPGRADED";
			}
			else if(($student->is_lock_manual == 1) && ($student->is_payment == 1)
					&& ($student->is_alloted == 1) && ($student->is_allotment_accept == 0) 
					&& ($student->is_upgrade_payment == 0) && ($student->is_upgrade == 0)&& ($student->s_admited_status == 0)) {
				$status = "SEAT_ALLOTED";
			}
			else {
		...
		*/
            if (($student->is_lock_manual == 1) /*&& ($student->is_payment == 1)*/
                && ($student->is_alloted == 1) && ($student->is_allotment_accept == 0)
                && ($student->is_upgrade == 0) && ($student->s_admited_status == 0)
            ) {
                $status = "SEAT_ALLOTED";
            } else if (($student->is_lock_manual == 1) /*&& ($student->is_payment == 1)*/
                && ($student->is_alloted == 1) && ($student->is_allotment_accept == 1)
                ...
                && ($student->is_alloted == 1) && ($student->is_allotment_accept == 0)
                && ($student->is_upgrade_payment == 0) && ($student->s_admited_status == 0)
            ) {
                $status = "UPGRADATION_PAYMENT_PENDING";
            } else if (($student->is_lock_manual == 1) /*&& ($student->is_payment == 1)*/
                && ($student->is_alloted == 1) && ($student->is_allotment_accept == 0)
                && ($student->is_upgrade == 1) && ($student->is_upgrade_payment == 1) && ($student->s_admited_status == 0)
            ) {
                $status = "ALLOTMENT_UPGRADED";
            } else {
                $status = "DATA_NOT_AVAILABLE";
            ...
            } else if (($student->is_lock_manual == 1) && ($student->is_payment == 0)) {
                $status = "CHOICE_MANUAL_LOCK";
            } else if (($student->s_home_district != NULL) && ($student->is_lock_manual == 0)) {
