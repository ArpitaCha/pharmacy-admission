...
<body>
    ...
    @else
        <style>
            body::before {
                ...
                /* adjust size */
                height: 450px;
                background: url("{{ public_path('storage/uploads/council_logo.png') }}") no-repeat center;
    ...
    <div class="header" style="position:fixed; top:"0px"; padding-bottom:3px;">
        <div class="logo-container"style="position:absolute;margin-top:5px;margin-left:-5px;">

            <img src="{{ public_path('storage/uploads/council_logo.png') }}" alt="Council Logo"
    ...
    <div>
                ...
                <tr>
                    ...
                    <td style="text-align:center;"">
                        @if (!empty($students->s_photo))
                            <img src="{{ public_path('storage/' . $students->s_photo) }}" alt="Student Photo"
                                style="width:100px; height:auto; border:1px solid #000; display:block; margin:0 auto 10px;">
                        @endif
                    </td>
                    <td style="text-align:center;">
                        @if (!empty($students->s_sign))
                            <img src="{{ public_path('storage/' . $students->s_sign) }}" alt="Student Signature"
        ...
        <table style="padding-top:5px;">
            ...
            <tr>
                ...
                <td>
                    @if ($students->s_gender == 'FEMALE')
                        {{ $students->s_kanyashree }}
                    @else
                        N/A
                    @endif
                </td>
        ...
        <table style="padding-top:5px;">
            <tr>
                <th colspan="4" style="text-align:left; font-size:14px;">Residential Details Of Address</th>
            </tr>
            <tr>
        ...
        @if ($students->is_business == '1')
            <table>
                ...
                <tr>
                    <td><strong>Name of Business or Profession of Pharmacy</strong></td>
                    <td>{{ $business_details->business_name ?? 'N/A' }}</td>
                    <td><strong>Address of Business or Profession</strong></td>
                    <td>{{ $business_details->business_address ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td><strong>Address 2 of Business or Profession</strong></td>
                    <td>{{ $business_details->business_address_2 ?? 'N/A' }}</td>
                    <td><strong>Post Office of Business or Profession</strong></td>
                    <td>{{ $business_details->business_post_office ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td><strong>Police Station of Business or Profession</strong></td>
                    <td>{{ $business_details->business_police_station ?? 'N/A' }}</td>
                    <td><strong>Block Name of Business or Profession</strong></td>
                    <td>{{ $business_details->block->name ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td><strong>Sub-Division of Business or Profession</strong></td>
                    <td>{{ $business_details->subdivision->name ?? 'N/A' }}</td>
                    <td><strong>District of Business or Profession</strong></td>
                    <td>{{ $business_details->district->district_name ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td><strong>State of Business or Profession</strong></td>
                    <td>{{ $business_details->state->state_name ?? 'N/A' }}</td>
                    <td><strong>PIN Code of Business or Profession</strong></td>
                    <td>{{ $business_details->business_pin_code ?? 'N/A' }}</td>
                </tr>
            </table>
        @endif
        <table style="padding-top:5px;">
            ...
            <tr>
                ...
                <td colspan="2"> {{ $students->s_pwd == 1 ? 'Yes' : 'No' }}
                    ...
                    @if ($students->s_pwd == 1)
                        ...
                        {{ \Carbon\Carbon::parse($students->pc_cert_date)->format('d-m-Y') }})</p>
                    @endif
                </td>
            </tr>
    ...
    <div style="page-break-before: always;">
        ...
        <table>
            <tr>
                <th colspan="5" style="text-align:left; font-size:14px;">Bank Account Details for refund of
                    successfull duplicate payment (if any) made by the applicant</th>
            </tr>
        ...
        <div style="position: fixed; bottom: 5px; left: 20px; right: 20px; font-size:12px; line-height:1.3;">
            ...
            <p style="margin:3px 0; text-align: justify; font-size:11px; line-height:1.3;">
                ...
                & belief and nothing has been concealed. I understand that submission of any false/incorrect information
                may lead to cancellation of my candidature from the admission process at any point of time, even after
                securing a position by me in the merit list, allotment of seat in my favour, taking provisional
                admission in any polytechnic and getting registered under the WBSCT&VE&SD.
            </p>
            ...
            <table style="width:100%; border:none; border-collapse:collapse; margin-top:6px;">
                <tr style="border:none;">
                    ...
                    <td style="width:50%; text-align:right; vertical-align:middle; border:none;">
                        @if (!empty($students->s_sign))
                            <img src="{{ public_path('storage/' . $students->s_sign) }}" alt="Signature"
            ...
            @if ($type = 'PREVIEW')
                <h3 style="color:red;"><span>Note:</span> Your Application will be considered only after successful
                    payment. </h3>
            @endif
